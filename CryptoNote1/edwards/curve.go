...
// ScalarMul multiplies two scalars and returns the product mod N.
func ScalarMu(a, b *big.Int) *big.Int {
	feA := BigIntToFieldElement(a)
	feB := BigIntToFieldElement(b)
	sum := new(edwards25519.FieldElement)

	edwards25519.FeMul(sum, feA, feB)
...
// ScalarAdd subbs two scalars and returns the difference mod N.
func ScalarSub(a, b *big.Int) *big.Int {
	feA := BigIntToFieldElement(a)
	feB := BigIntToFieldElement(b)
	sum := new(edwards25519.FieldElement)

	edwards25519.FeSub(sum, feA, feB)
	sumArray := new([32]byte)
	edwards25519.FeToBytes(sumArray, sum)
