...
func randPrivScalarKeyList2(curve *TwistedEdwardsCurve, i int) []*PrivateKey {

	privKeyList := make([]*PrivateKey, i)
	for j := 0; j < i; j++ {

		privKeyList[j], _ = GeneratePrivateKey(curve)

		cu := elliptic.Curve(edwards.Edwards())
		x,y:= cu.ScalarBaseMult(privKeyList[j].GetD().Bytes())
		fmt.Printf("\n origi %v -- vs new %v\n",x , curve.IsOnCurve(x,y))
	}
...
func TestSign(t *testing.T) {
	...
	x,y:=curve.ScalarBaseMult(p[0].GetD().Bytes())
	fmt.Printf("\n%v %v", x, y)
	//p.
}
...
func TestNonStandardSignatures(t *testing.T) {
	...
	pks := randPrivScalarKeyList2(curve, 150)
	for _, pk := range pks {
